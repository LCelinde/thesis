...
#delete projects in other languages than english (from 241 to 30 projects)
...
#delete rows without discriptions or too much spelling errors (probably other language)(from 517 to 500 projects)
...
# Boxplot of accuracy and error
boxplot(acc,col='pink',border='black',horizontal=T,xlab='Accuracy',
        main='Classification accuracy CV')
boxplot(mce,col='pink',border='black',horizontal=T,xlab='Error',
        main='Classification error CV')
...
res.best.logistic <-
  bestglm(Xy = data_for_best_subset,  family = binomial,
          IC = "CV", 
...
fit_glm3 <- glm(Success ~ log_USD_Goal + Numberofcomments, 
             data = data_projects_new, family = binomial(link = "logit"))
summary(fit_glm3)
...
for(fold in 1:K){
  ...
  # Fitting
  model <- glm(Success ~ log_USD_Goal + Numberofcomments, 
               data = train, family = binomial(link = "logit"))
...
table(data_projects_new$Success)
55/(412+55)
...
fit_kick <- rpart(Success ~ Prototypicality + misspelledwords + FOGindex 
                  ...
                  parms = list(prior = c(.8822, .1178), loss = c(0, 2, 2, 0)), control = list(rpart.control))
